...
class Taxonomy_Switcher_UI {
	...
	public function add_page() {
		...
		$this->options_page = add_options_page( $this->admin_title, $this->admin_title, 'manage_options', $this->admin_slug, array( $this, 'do_page' ) );
	...
	public function do_page() {
		...
		<form method="get">
			...
			<table class="form-table"><tbody>
				...
				<tr valign="top">
					<th scope="row"><label for="taxonomy-switcher-parent"><?php _e( 'Limit taxomoy switch for child terms of a specific parent', 'wds' ); ?></label></th>
					<td>
	...
	public function fill_options( $name ) {
		...
		foreach ( $this->registered_taxonomies as $slug => $tax_object ) {
			echo '<option value="'. $slug .'" '. selected( $slug, $current ) .'>'. $tax_object->labels->name .'</option>';
		}
	}
	...
	public function send_error( $line, $msg = '' ) {
		...
		wp_send_json_error( array( 'html' => '<ul><li>'. $msg .'</li></ul>', 'line' => $line, '$_REQUEST' => $_REQUEST ) );
	}
